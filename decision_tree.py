...
class DecisionTree(Model):
    ...
    def impurity(self, left_label_hist, right_label_hist):
        ...
        H_l = sum(map(lambda value: (value / n_l) * log(value / n_l), left_label_hist.values()))
        H_r = sum(map(lambda value: (value / n_r) * log(value / n_r), right_label_hist.values()))
    ...
    def find_best_split_rule(self, data, labels):
        ...
        for feature_index in valid_features:
            for threshold in self.possible_threshold_values[feature_index]:
                ...
                left_label_hist, right_label_hist = self.get_histograms(data, labels, candidate_split_rule)
                impurity_value = self.impurity(left_label_hist, right_label_hist)
                if (min_impurity_value is None) or (impurity_value < min_impurity_value):
    ...
    def train(self, data, labels, max_label = None, remaining_height = None):
        ...
        if remaining_height > 0:            
            ...
            left_data, left_labels, right_data, right_labels = self.split_data_and_labels(data, labels, node.split_rule)
            
            if len(left_data) == 0 or len(right_data) == 0:
                # If there would be no more data to split in one of the child nodes, make a leaf
                self.make_leaf(node, labels, max_label)
            else:
                node.left = self.train(left_data, left_labels, max_label, remaining_height - 1)
                node.right = self.train(right_data, right_labels, max_label, remaining_height - 1)
    ...
    def make_leaf(self, node, labels, max_label):
        ...
        for label in labels:
            ...
            if counts[l] > max_count:
                max_count = counts[l]
                mode_label = l
        node.distribution = np.multiply(float(1) / k, counts).reshape((1, distribution_length))
    ...
    def predict_for_forest(self, data, report_common_splits = False, tree_num = 0):
        ...
        if report_common_splits:
            ...
            if split_rule.is_categorial:
                ...
                operator = ">" if passed else "<="
            print("Most common split at tree #" + str(tree_num) + ": feature #" + str(split_rule.feature_index) +" " + operator + " " + str(split_rule.threshold))
        return dist_pred
...
class TreeNode:
    def __init__(self, left, right, split_rule, label):
        ...
        self.distribution = {} # Belief distribution over the correct classification of labels that arrive
                               # at this node.
