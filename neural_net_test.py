...
def report(predictions_validation, predictions_training, labels, validation_indices, training_indices):
    ...
    training_accuracy = get_frac_correct(training_indices, predictions_training)
    validation_accuracy = get_frac_correct(validation_indices, predictions_validation)
...
if __name__ == "__main__":
    ...
    training_indices = np.random.choice(SAMPLE_SIZE, size=TRAINING_SIZE, replace=False)
    validation_indices = np.setdiff1d(np.arange(SAMPLE_SIZE), training_indices)
    random.shuffle(training_indices)
    data = pre_process(X_train)

    neural_net = NeuralNet()
    iterations, losses, accuracies = neural_net.train(data, labels_train, training_indices)
    ...
    report(predictions_validation, predictions_training, labels_train, validation_indices, training_indices)
